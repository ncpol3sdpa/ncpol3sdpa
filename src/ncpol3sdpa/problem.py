...
from ncpol3sdpa.rules import Rule, apply_rule_to_polynom, apply_rule
...
def polynomial_to_matrix(
    algebra: momentmatrix.AlgebraSDP, poly: sp.Expr
) -> NDArray[np.float64]:
...
def algebra_to_SDP_add_equality_constraint(
    problem: ProblemSDP, algebra: momentmatrix.AlgebraSDP, eq_constraint: sympy.Expr
) -> None:
...
def algebra_to_SDP_add_inequality_constraint(
    ...
    constraint_moment_matrix: List[List[sp.Expr]],
) -> None:
...
class Problem:
    ...
    def solve(self, relaxation_order: int = 1) -> float:
        ...
        all_constraint_polynomials = [c.polynom for c in self.constraints] + \
                                     [self.objective]
        needed_symbols = momentmatrix.generate_needed_symbols(
    ...
    def solve_old(self, relaxation_order: int = 1) -> float:
        ...
        all_monomials: List[Expr] = [
            monom
            for monom in generate_monomials_commutative(all_symbols, relaxation_order)
        ...
        constraint_maricies_positive = [
            create_constraints_matrix_cvxpy(
                ...
                generate_monomials_commutative(
                    ...
                    int(
                        relaxation_order
                        - (total_degree(self.constraints[i].polynom) / 2)
                    ),
                ),
