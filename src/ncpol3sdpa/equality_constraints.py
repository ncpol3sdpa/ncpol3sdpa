...
def rule_of_constraint(constraint):
    ...
    max_degree = constraint.polynom.total_degree()    # degree of the polynom seen as a multivariable polynom

    leader_monomials = [monom for monom in polynom if sum(monom[0]) == max_degree]    # list of monom that maximize the degree of the polynom
    ...
    for monom_index in range(len(constraint.polynom.gens)):
        leader_monomial_expressed *= constraint.polynom.gens[monom_index]**leader_monomial[0][monom_index]
