...
class ConstraintType(Enum):
    ...
    EQUALITY = auto()
    """EQUALITY: Represents an equality constraint of the form p = 0"""
    INEQUALITY = auto()
    """INEQUALITY: Represents an inequality constraint of the form p >= 0"""
    LOCAL_INEQUALITY = auto()
    """LOCAL_INEQUALITY: Represents a local inequality constraint of the form p > 0"""
...
class Constraint(metaclass=NoPublicConstructor):
    ...
    def EqualityConstraint(cls, polynomial: Expr | Poly) -> Constraint:
        """Create an equality constraint of the form polynomial = 0.
    ...
    def LocalInequalityConstraint(cls, polynomial: Expr) -> Constraint:
        ...
        Notes
        -----
        * In the non-commutative case: The constraint is `Trace(ρ polynomial) >= 0`
        """
