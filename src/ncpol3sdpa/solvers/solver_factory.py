...
class SolverFactory:
    ...
    @classmethod
    def create_solver(cls, solver_name: str | SolverList, config: Optional[Dict[str, Any]] = None) -> Solver:
        """
        ...
        if isinstance(solver_name, SolverList):
            ...
            raise ValueError(f"Invalid solver name type: {type(solver_name)}. Expected str or SolverList.")

        if solver_class is None:
            raise ValueError(f"Unknown solver: {solver_name}. Solvers included: {list(cls._solver_map.keys())}")
